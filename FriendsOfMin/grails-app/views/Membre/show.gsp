...
<html>
	...
	<body>
					<div style="position: absolute;left: 60%;margin: 100px;">
	<g:mypic member="${membreInstance}"/>
	</div>
		...
		<div id="show-membre" class="content scaffold-show" role="main">
			...
			<ol class="property-list membre">
				...
				<g:if test="${membreInstance?.age}">
				<li class="fieldcontain">
					<span id="age-label" class="property-label"><g:message code="membre.age.label" default="Age" /></span>
					
						<span class="property-value" aria-labelledby="age-label"><g:fieldValue bean="${membreInstance}" field="age"/></span>
				...
				<li class="fieldcontain">
					<span id="passions-label" class="property-label"><g:message code="membre.passions.label" default="Passions" /></span>
						...
						<span class="property-value" aria-labelledby="passions-label"><g:link controller="passions" action="show" id="${p.id}">${p?.encodeAsHTML()}</g:link></span>
						</g:each>
					
				</li>
				</g:if>
			
				<g:if test="${membreInstance?.photos}">
				<li class="fieldcontain">
					<span id="photos-label" class="property-label"><g:message code="membre.photos.label" default="Photos" /></span>
					
						<g:each in="${membreInstance.photos}" var="p">
						<span class="property-value" aria-labelledby="photos-label"><g:link controller="photoTaggee" action="show" id="${p.id}">${p?.encodeAsHTML()}</g:link></span>
						</g:each>
