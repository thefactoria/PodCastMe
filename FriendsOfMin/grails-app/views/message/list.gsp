			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="messages" title="${message(code: 'message.messages.label', default: 'Messages')}" />
					
						<g:sortableColumn property="date" title="${message(code: 'message.date.label', default: 'Date')}" />
					
						<th><g:message code="message.env.label" default="Env" /></th>
					
						<th><g:message code="message.rec.label" default="Rec" /></th>
				...
				<g:each in="${messageInstanceList}" status="i" var="messageInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${messageInstance.id}">${fieldValue(bean: messageInstance, field: "messages")}</g:link></td>
					
						<td><g:formatDate date="${messageInstance.date}" /></td>
					
						<td>${fieldValue(bean: messageInstance, field: "env")}</td>
					
						<td>${fieldValue(bean: messageInstance, field: "rec")}</td>
